...
async def get_datas():
    try:
        async with async_playwright() as p:
            ...
            await page.goto(
                'https://margarittotash.salesdoc.io/report/reportBuilder/getResult?reportType=order&datestart=2024-06-13&endstart=2024-06-18&bydate=DATE_LOAD&status%5B%5D=2&status%5B%5D=3&sum=on&count=on&akb=on&field=%5B%22date%22%2C%22client%22%2C%22city%22%2C%22agent%22%2C%22product%22%2C%22productCat%22%5D')

            logging.info('Extracting report data')
            data = page.locator('pre')
            if not data:

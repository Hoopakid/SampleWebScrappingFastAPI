...
async def getting_data():
    ...
    response = requests.get(
        f'https://margarittotash.salesdoc.io/report/reportBuilder/getResult?reportType=order&datestart=2024-05-01&endstart=2024-05-09&bydate=DATE&status%5B%5D=2&status%5B%5D=3&sum=on&volume=on&akb=on&field=%5B%22date%22%2C%22client%22%2C%22city%22%2C%22agent%22%2C%22product%22%2C%22productCat%22%5D',
...
async def hayot_data():
    async with async_playwright() as p:
        browser = await p.chromium.launch(headless=False)
        context = await browser.new_context()
        page = await context.new_page()
        await page.goto('https://hayotporcelain.salesdoc.io/site/login')
        username = page.locator('[name="LoginForm[username]"]')
        await username.fill('Elbek1991')
        password = page.get_by_placeholder('Пароль')
        await password.fill('6666899100')
        button = page.get_by_role('button')
        await button.click()
        sleep(7)
        cookies = await context.cookies()
    ...
    response = requests.post(
        f'https://margarittotash.salesdoc.io/report/reportBuilder/getResult?reportType=order&datestart=2024-05-01&endstart={today}&bydate=DATE&status%5B%5D=2&status%5B%5D=3&sum=on&volume=on&akb=on&field=%5B%22date%22%2C%22client%22%2C%22city%22%2C%22agent%22%2C%22product%22%2C%22productCat%22%5D',
        cookies={'PHPSESSID': cookie})
    df = pd.DataFrame(response.json())
    df.to_csv('data.csv', index=False)
    data = pd.read_csv('data.csv')

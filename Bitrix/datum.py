            ...
            if isinstance(value, dict):
                ...
                for offset, val in enumerate(value):
                    if isinstance(val, dict):
                        ret += prepare_params(val, "{0}[{1}][{2}]".format(prev, key, offset))
                    else:
                        if prev:
...
def create_batch(method, params: dict = {}):
    params['start'] = 0
    r = requests.get(f'{BITRIX_URL}{method}', params=prepare_params(params))
...
async def get_contact_by_id(contact_id: int):
    ...
    for batch in batches:
        r: dict = await btx.call_batch(batch)
...
async def get_user_by_id(user_id: int):
    ...
    for batch in batches:
        temp: dict = await btx.call_batch(batch)
...
async def get_deal_by_id(deal_id: int):
    ...
    contact = await get_contact_by_id(deal_info[0]['CONTACT_ID'])
    return contact
...
async def get_calls_fast():
    ...
    btx = Bitrix(BITRIX_URL)
    for batch in batches:
        resp = await btx.call_batch(batch)
        for key, val in resp.items():
            calls.extend(val)
    for call in calls:
        call['contact'] = await get_deal_by_id(call['OWNER_ID'])
        author_id = call['AUTHOR_ID']
        user = await get_user_by_id(author_id)
